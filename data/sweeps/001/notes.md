...
Temp DB path was `backtest/results/sweep-NNNN.db`. Stale `.db` files from
earlier test sweeps (`test-sweep.csv`, `test-sweep-2.csv`) remained at that path.
The `BankrollManager` constructor recovers balance from existing DB
(`db.getLatestBalance()`), so each sweep iteration started with inflated balance
from the previous test run rather than $200. Evidence:

- Trade counts (301, 450, 514) far exceed the 107-238 range produced by clean
  runs with the same parameters
- A standalone backtest with identical params (0.001, 0.45, 0.05) produces
  PnL=-$9.58 / 107 trades, matching sweep 002, not sweep 001
- HWM values ($18,977 for top combo) are impossible from $200 start in 189h
  without balance inflation
