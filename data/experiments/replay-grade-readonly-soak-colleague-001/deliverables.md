...
Verdict: `passed=true` in `manifest.json`. Every hard former root handoff gate satisfied:
...
* Builder relayer creds intentionally absent → `redemption_readiness=unavailable_missing_builder_credentials` (the former root handoff said fail-closed redemption is acceptable on a readonly soak).
