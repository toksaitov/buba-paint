...
Independent no-order `live_readonly` soak attempt against `buba-paint-fin` per the former root handoff. Scope: paper trading plus authenticated CLOB v2 readonly account/order reads. No `live_trading`, no arming, no orders, no cancels, no redemptions.
