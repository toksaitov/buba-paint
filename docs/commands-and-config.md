...
`live_trading` starts disarmed. Do not treat the presence of live caps, sidecar credentials, callable sidecar write endpoints, or queued `live-control` commands as permission to deploy real money. The live-money canary is prepared on `master` and staged disarmed on the live host; arming requires an explicit operator GO (see [../LIVE_READINESS_PLAN.md](../LIVE_READINESS_PLAN.md) and [../CANARY_RUNBOOK.md](../CANARY_RUNBOOK.md)).
