...
* Live dry-run: `LIVE_DRY_RUN` (default false) rehearses the full armed `live_trading` path. Every gate, signal, decision, intent persistence, and idempotency reservation runs, and the real order request is built, but the sidecar order endpoint is never called: the worker logs a would-submit line plus a durable `dry_run_would_submit` reconciliation event, releases the reservation, and produces no venue order, fill, or on-chain reconciliation. Use it to rehearse the canary sequence with zero real money; it must be explicitly false for a real canary.
* Live session order cap: `LIVE_MAX_SESSION_ORDERS` (default 0, meaning unlimited) structurally bounds a live session to that many venue submission attempts. The cap is enforced atomically inside the venue-attempt reservation (a single guarded SQL update that checks the counts and marks the intent in one serialized write), so it holds regardless of operator reaction time or concurrent workers, and it counts dry-run attempts too. It fails closed.
* Live session fill cap: `LIVE_MAX_SESSION_FILLS` (default 0, meaning unlimited) stops further venue submissions once that many orders in the session have filled, enforced atomically in the same reservation. The canary sets `LIVE_MAX_SESSION_ORDERS=1` and `LIVE_MAX_SESSION_FILLS=1` so it places exactly one venue submission and does not retry a no-fill. Bootstrap also fails closed on any prior `live_orders` row and on an orphaned attempt (status `submitting` with no `live_orders` row), so across a restart a fill can never be orphaned or doubled; the full set of restart bails is described in the sidecar paragraph below. See [canary-config.md](./canary-config.md) for the full reversible canary overlay.
* Live armed feed-outage halt: `LIVE_ARMED_FEED_OUTAGE_HALT_MS` (default 120000; 0 disables) halts an armed live session when a critical decision feed (Binance or CLOB) is in a continuous outage longer than this. Brief reconnect blips only block orders (no order is placed on stale data); a sustained outage halts the session. Inert when disarmed.
...
That policy is the funded baseline. The design steady state is `live_readonly`; the live host is currently staged off it in the disarmed, parked `live_trading` canary for the live-readiness effort (see [../LIVE_READINESS_PLAN.md](../LIVE_READINESS_PLAN.md)).
