...
* Live session fill cap: `LIVE_MAX_SESSION_FILLS` (default 0, meaning unlimited) stops further venue submissions once that many orders in the session have filled, enforced atomically in the same reservation. With the order cap this gives the canary "a few attempts to land one fill, then stop": set `LIVE_MAX_SESSION_ORDERS` to a small number for no-fill retries and `LIVE_MAX_SESSION_FILLS=1` so exactly one filled position can occur. See [canary-config.md](./canary-config.md) for the full reversible canary overlay.
