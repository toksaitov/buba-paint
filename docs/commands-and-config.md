...
* Live session order cap: `LIVE_MAX_SESSION_ORDERS` (default 0, meaning unlimited) structurally bounds a live session to that many venue submission attempts. The cap is enforced atomically inside the venue-attempt reservation (a single guarded SQL update that checks the count and marks the intent in one serialized write), so the canary value of 1 limits the session to exactly one order regardless of operator reaction time or concurrent workers, and it counts dry-run attempts too. It fails closed.
