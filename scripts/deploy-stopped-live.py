...
def deploy_remote_script(root: str, release: str, lock: dict[str, Any], token: str) -> str:
    ...
    return "\n".join(
        [
            ...
            "research_work_dir=\"$root/research-work\"",
            "mkdir -p \"$research_work_dir/artifacts\"",
            "tmp_env=$(mktemp)",
            "grep -v -E '^(BUBA_RESEARCH_WORK_DIR|BUBA_RESEARCH_WORK_ROOT)=' \"$release_env\" > \"$tmp_env\"",
            "printf 'BUBA_RESEARCH_WORK_DIR=%s\\nBUBA_RESEARCH_WORK_ROOT=/research\\n' \"$research_work_dir\" >> \"$tmp_env\"",
            "cat \"$tmp_env\" > \"$release_env\"",
            "rm -f \"$tmp_env\"",
