...
* `probe-venue-assumptions.py`: read-only Phase 0 live-readiness probe; calls the sidecar `/preflight` and writes a no-secret venue-assumptions artifact (fee rate and exponent, min size, tick size, collateral, signature type, hosts, exchange domain version) to an ignored `/tmp` path. Run at the operator gate against a sidecar configured with real read-only credentials.
