...
- Phase 10B, True Bot Hot-Path Isolation and Low-Latency Readiness: implemented locally on 2026-05-08, commit pending. This phase responds to the Docker run starvation failure by moving replay-grade feed persistence, live submission, readonly account refresh, live control refresh, remote sidecar polling, market persistence, feed-health persistence, and runtime metadata writes out of the feed loop. Blockers fixed: runtime replay validators, Docker `quick_check`, direct `tick_data` logging, feed-path DB writes, feed-path DB reads, and main-loop sidecar/control awaits were removed from latency-sensitive paths. Accepted debt: local Docker 60-90 minute soak remains to be run after the current local dashboard collaboration is no longer using the machine.
