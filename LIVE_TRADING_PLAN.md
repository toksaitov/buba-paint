...
- Phase 10C, Pure Decision Path and Compact Decision-Input Capture: implemented locally on 2026-05-08, commit pending. This phase replaces the DB-backed runtime strategy worker with a pure in-memory decision worker and a bounded persistence writer. Blockers fixed: live/paper runtime decisions no longer call `StrategyWorker`, `run_strategy_cycle`, `ExecutionEngine`, signal persistence, open-trade scans, or paper-order DB processing before returning a decision; live pending intents now reserve exposure in memory until submission feedback resolves them; replay-grade capture preserves decision-critical typed sequencing while stripping bulky payloads. Accepted debt: the long local Docker `live_readonly` soak remains to be run when the local dashboard collaboration is not using the machine.
