...
- Phase 10B, True Bot Hot-Path Isolation and Low-Latency Readiness: complete in commit `3aded7c` on 2026-05-08. This phase responds to the Docker run starvation failure by moving replay-grade feed persistence, live submission, readonly account refresh, live control refresh, remote sidecar polling, market persistence, feed-health persistence, and runtime metadata writes out of the feed loop. Blockers fixed: runtime replay validators, Docker `quick_check`, direct `tick_data` logging, feed-path DB writes, feed-path DB reads, and main-loop sidecar/control awaits were removed from latency-sensitive paths.
- Phase 10C, Pure Decision Path and Compact Decision-Input Capture: complete in commit `63354b7` on 2026-05-08. This phase replaces the DB-backed runtime strategy worker with a pure in-memory decision worker and a bounded persistence writer. Blockers fixed: live/paper runtime decisions no longer call `StrategyWorker`, `run_strategy_cycle`, `ExecutionEngine`, signal persistence, open-trade scans, or paper-order DB processing before returning a decision; live pending intents now reserve exposure in memory until submission feedback resolves them; replay-grade capture preserves decision-critical typed sequencing while stripping bulky payloads. Accepted debt: the long local Docker `live_readonly` soak remains to be run when the local dashboard collaboration is not using the machine.
- Phase 10D, Live Decision Correctness Closure and Evidence Integrity Gate: complete in commit `c9df981` on 2026-05-09. This phase removes the legacy direct live-submission path, models live submission feedback from actual accepted size and fill price, keeps unknown/timeout outcomes blocked, makes same-output evidence/submission queue failures fail closed, and wires compact decision evidence into persisted signal metadata.
- Phase 10E, Critical Evidence Durability and Worker Closeout Reliability: complete in commit `4bd1b7d` on 2026-05-09. This phase makes critical live decision evidence durable with the live intent before sidecar submission, bounds worker closeout, separates live-control priority from remote refresh, and keeps persistence failures fail-closed.
- Phase 10F, Stale-Decision Backpressure and Evidence Completeness Closure: implemented locally on 2026-05-09, commit pending. This phase replaces feed-derived FIFO strategy backlog with latest-state decision semantics, adds stale-output gates, bounds runtime metadata/feed-health persistence, and completes live decision evidence context. Blockers fixed: stale feed-derived strategy backlogs, unbounded decision outputs, ad-hoc runtime metadata/feed-health DB tasks, and incomplete live decision evidence.
...
- Implemented observed replay-quality metadata semantics. Runtime code records `runtime_observed_replay_quality_class` and recent capture health from incremental counters; the full `replay_quality_class` key is reserved for offline validation and closeout evidence.
