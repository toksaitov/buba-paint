...
Data: `bankroll.rs` (per-strategy half-Kelly sizing, sleeves, caps, confidence curve, DD pause), `position_manager.rs` (trade lifecycle: open with debug logging on every rejection path, guard duplicates, authoritative resolve), `circuit_breaker.rs` (pause trading after N consecutive losses), `tick_logger.rs` (1s telemetry sampling to SQLite), `trend_tracker.rs` (strategy-scoped directional trend filter), `market_discovery.rs` (Gamma API polling, slug-based window discovery, resolution polling for deferred settlement), `portfolio.rs` (regime router, family attribution, and non-competing portfolio helpers).
...
- Portfolio routing: when `REGIME_DETECTION_ENABLED=1`, one evaluation snapshot is classified as `dislocation`, `structural_pair`, or `calm`. Only the matching family should act on that snapshot. Strategy sleeves such as `LATENCY_ARB_MAX_POSITION_FRACTION`, `SPREAD_CAPTURE_MAX_POSITION_FRACTION`, and `CALM_PERSISTENCE_MAX_POSITION_FRACTION` keep one family from consuming another family's budget.
