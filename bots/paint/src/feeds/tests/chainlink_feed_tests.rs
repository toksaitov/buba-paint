...
fn parse_initial_data_dump_returns_all_entries() {
    // Verify the last entry matches the expected latest.
    let msg = serde_json::json!({
        "payload": {
            "data": [
                {"value": 41999, "timestamp": 122},
...
fn parse_chainlink_null_timestamp() {
    // When timestamp is null, it should default to 0.
    let msg = serde_json::json!({
        "topic": "crypto_prices_chainlink",
        "payload": {
            "value": 42000,
...
fn parse_chainlink_nested_empty_array() {
    // Outer array wrapping an inner payload with data=[].
    // Should return empty vec since there are no valid entries.
    let text = serde_json::to_string(&serde_json::json!([
        {
            "payload": {
                "data": []
