...
const READONLY_POLL_INTERVAL_SECS: u64 = 60;
...
/// Run the readonly authenticated venue runtime.
...
pub async fn run_live_readonly(
    ...
    info!(
        ...
        "starting live readonly runtime"
    );
    ...
    let mut session_state = ReadonlySessionState {
        session_id,
        status: "readonly_starting".to_string(),
    ...
    for event in readonly_reconciliation_events(session_id, None, &initial_account) {
        db.log_live_reconciliation_event(&event)?;
    }

    update_session_status(&db, &mut session_state, &config)?;

    let mut shutdown_rx = shutdown_rx;
    let mut previous_account = initial_account;
    let mut interval = tokio::time::interval(Duration::from_secs(READONLY_POLL_INTERVAL_SECS));
    ...
    loop {
        tokio::select! {
            _ = &mut shutdown_rx => {
                let ended_at_ms = clock.now();
                let details = session_details_json(
                    Some(&session_state.preflight),
                    session_state.account.as_ref(),
                    &session_state.soft_issues,
                    "readonly_stopped",
                );
                db.finish_live_session(session_id, ended_at_ms, "readonly_stopped", Some(&details))?;
                db.log_control_audit(&ControlAuditEntry {
                    id: None,
                    timestamp_ms: ended_at_ms,
                    actor: "system".to_string(),
                    action: "readonly_session_stopped".to_string(),
                    target: Some(session_id.to_string()),
            ...
            _ = interval.tick() => {
                match sidecar.account_state().await {
                    Ok(account) => {
                        db.log_live_account_snapshot(&account_snapshot(session_id, &account))?;
                        for event in readonly_reconciliation_events(session_id, Some(&previous_account), &account) {
                            db.log_live_reconciliation_event(&event)?;
                        }
                        previous_account = account.clone();
                        session_state.account = Some(account);
                        session_state.soft_issues = readonly_soft_issues(
                    ...
                    Err(error) => {
                        ...
                        db.log_live_reconciliation_event(&LiveReconciliationEvent {
                            ...
                            local_value: None,
                            remote_value: None,
                            details_json: Some(json!({ "error": error.to_string() }).to_string()),
                        })?;
                        session_state.soft_issues =
                            vec![format!("Readonly account refresh failed: {error}")];
                        update_session_status(&db, &mut session_state, &config)?;
                    }
                }
            }
...
fn readonly_soft_issues(
    ...
    if let Some(account) = account {
        ...
        if let Some(legal_order_min) = details.legal_order_min_usd {
            if legal_order_min > config.live_max_single_order_usd + CASH_CHANGE_EPSILON_USD {
                issues.push(format!(
                    "Current legal order minimum {:.2} exceeds LIVE_MAX_SINGLE_ORDER_USD {:.2}.",
                    legal_order_min, config.live_max_single_order_usd
...
mod tests {
    ...
    /// Verifies that readonly startup creates a ready session and account snapshot.
    #[tokio::test]
    async fn run_live_accepts_live_readonly_mode() {
        ...
        let task = tokio::spawn(async move {
            crate::live::run_live(config, &db_path_for_task, 100.0, shutdown_rx).await
        });
        tokio::time::sleep(Duration::from_millis(50)).await;
        let _ = shutdown_tx.send(());
        task.await.unwrap().unwrap();

        let conn = rusqlite::Connection::open(Path::new(&db_path)).unwrap();
        ...
        let snapshot_count: i64 = conn
            ...
            .unwrap();
        let simulated_trade_count: i64 = conn
            .query_row("SELECT COUNT(*) FROM simulated_trades", [], |row| {
                row.get(0)
            })
            .unwrap();
        ...
        assert_eq!(simulated_trade_count, 0);
    }

    /// Verifies that readonly startup still refuses live_trading.
    #[tokio::test]
    async fn run_live_refuses_live_trading_mode() {
        ...
        let error = crate::live::run_live(config, "/tmp/does-not-matter.db", 100.0, shutdown_rx)
            .await
            .unwrap_err()
            .to_string();
    ...
    #[tokio::test]
    async fn run_live_readonly_fails_on_hard_preflight_blockers() {
        let server = MockServer::start().await;
        Mock::given(method("POST"))
            .and(path("/preflight"))
        ...
        let error = crate::live::run_live(config, &db_path, 100.0, shutdown_rx)
            .await
            .unwrap_err()
            .to_string();
